...
final liveInfoProvider = StateNotifierProvider<LiveInfoStateNotifier, List<UserLiveInfoModel>>((ref) {
  final repository = ref.watch(liveInfoRepositoryProvider);
  final notifier = LiveInfoStateNotifier(repository: repository);
  return notifier;
});

class LiveInfoStateNotifier extends StateNotifier<List<UserLiveInfoModel>> {
  final LiveInfoRepository repository;
  LiveInfoStateNotifier({required this.repository}) : super([]) {
